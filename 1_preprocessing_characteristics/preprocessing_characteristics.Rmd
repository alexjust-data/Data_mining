...
author: "Autor: Alex Just Rodriguez"
date: "marzo 2022"
output:
  html_document:
    highlight: default
...
Este módulo cubre "El proceso de minería de datos" y "Preprocesado de los datos y gestión de características".

## Objetivos

Asimilar qué es y que no es Modelado de Datos. Ciclo de vida de los proyectos de MD. Diferentes tipologías de MD. Conocer las técnicas propias de una fase de conocimiento, preparación de datos y objetivos a lograr.
...
Se ha seleccionado un conjunto de datos del [National Highway Traffic Safety Administration](https://www.nhtsa.gov/). El sistema de informes de análisis de mortalidad fue creado en los Estados Unidos por la National Highway Traffic Safety Administration para proporcionar una medida global de la seguridad en las carreteras. (Fuente Wikipedia). Los datos pertenecen al año 2019. Se trata de un conjunto de registros de accidentes que recogen datos significativos que los describen. Todos los accidentes tienen alguna víctima mortal como mínimo. El objetivo analítico que tenemos en mente es entender que hace que un accidente sea grave y que quiere decir que sea grave.
...
Vemos que no hay valores nulos en los datos. También verificamos si existen campos llenos de espacios en blanco. En este caso sí encontramos el campo TWAY_ID2 con 24741 valores en blanco. Valoramos no hacer ninguna acción de eliminar registros puesto que este campo no lo utilizaremos.
...
El número de peatones implicados (PEDS) es muy bajo siendo coherente con el tipo de vía que se estudia y dónde no es habitual que haya gente andando. Con todo el valor como media de 0,23 y máximo de 9 obliga a investigar más este dato. (PERSONS) El número de ocupantes de vehículo implicados se sitúa como media en 2,24 (PERMVIT) El número conductores y ocupantes de vehículos en circulación implicados tiene un valor de media de 2,22. Estas dos variables recogen la misma información, pero la cuantifican de diferente manera. El accidente con el mayor número de ocupantes es de 74 personas. Como curiosidad hay cuatro accidentes donde no hay conductores implicados, pero si vehículos. No sabemos si es un error o un accidente extraño. Por lo que respecta al número peatónes, ciclistas, personas en vehículos aparcados y otros (PERNOTMVIT) vemos que aumenta un poco la media respecto a peatón puesto que entendemos que se incluyen más casos.
...
ylab = "Porcentaje",
main = "Distribución de accidentes por muertes y conductores positivos en alcohol",
...
ggplot(data=accidentDataST5[1:files,],aes(x=DRUNK_DR,fill=STATENAME))+geom_bar()+ggtitle("Relación entre las variables conductor bebido y Estado")+labs(x="Número de conductores bebidos implicados")
```
...
Como se puede observar la función summary, nos devuelve la proporción de varianza aplicada al conjunto total de cada atributo. Gracias a esto, el atributo 1 explica el 0.5795 de variabilidad del total de datos; en cambio, el atributo 7 explica solo el 0.00498.
...
-   También podemos limitar el número de componentes a este número que representa una determinada fracción de la varianza total. Por ejemplo, si estamos satisfecho con el 70% de la varianza total explicada, usamos el número de componentes para conseguirlo que son los 4 componentes principales vistos antes.
...
Podemos afirmar que a lo largo del 2019 en las autopistas de EE.UU. sucedieron 33.244 accidentes en los que perdieron la vida 36.096 personas. Pretendiamos extraer relaciones entre la presencia de alcohol en los conductores y el número de accidentes, pero las conclusiones no fueron claras. Las relaciones más obvias comprobadas son el incremento de muertes en función del incremento del número de vehículos, pasajeros y peatones implicados.
...
Estudiando el número de muertes en accidente en relación con el estado donde ha sucedido y la condición climática, vemos necesario profundizar con técnicas por ejemplo de agregación para ver como se agrupan y poder obtener un perfil.
